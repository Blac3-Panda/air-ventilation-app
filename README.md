...
## 사용 기술
- Frontend: Flutter (Dart, Material 3), `http`, `shared_preferences`, `google_fonts`, `url_launcher`
- Backend: FastAPI (Python), `uvicorn`, `requests`, `python-dotenv`
- External APIs:
  - 한국환경공단 에어코리아 대기오염정보
  - 기상청 초단기실황 API
...
4. 변환된 위경도를 기상청 격자(nx, ny)로 변환합니다.
5. 에어코리아(대기질), 기상청(풍향/풍속) 데이터를 수집합니다.
6. 주소 토큰 기반으로 측정소를 선택하고 위험 점수를 계산합니다.
7. 결과(`riskScore`, `recommendation`, `weather`, `air`, `grid`)를 프론트로 반환합니다.
